...
const AdminOrder = ({ id }) => {
  ...
  const checkStatus = (status) => {
    if (status === filterValue) {
      ...
      const filter = status
        ? data.filter((items) => items.orderStatus === status)
        : data;
          ...
          return (
            <button
              ...
              className={`${
                status === filterValue
                  ? "bg-violet-700"
                  : "bg-fudo-red hover:bg-[#e64747]"
              }  text-white inline-flex items-center focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2.5 text-center h-12 mr-5 my-auto`}
